...
class PrefillUserMixin:
    ...
    def prefill_user_fields(self, user):
        ...
        self.initial.update(
            {"email": self.user.email, "name": self.user.get_full_name() or self.user.username}
        )
...
class AbstractContactForm(forms.ModelForm):
    ...
    def get_field_summary(self):
        ...
        return OrderedDict(
            [
                (force_str(field.label), field.value())
                for field in self.visible_fields()
                if field.name not in self.hide_summary_fields
            ]
        )
